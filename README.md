...
Dokumentasi lengkap pengembangan, instalasi, deployment (cPanel & aaPanel), pembaruan (_upgrade frontend/backend_), serta panduan pemecahan masalah (_troubleshooting_) untuk sistem portal **BPVP Pangkep**.
...
7. [Panduan Integrasi Google Spreadsheet (Modul Shortlink)](#-panduan-integrasi-google-spreadsheet-modul-shortlink)
8. [Daftar Perintah Penting (Cheat Sheet)](#-daftar-perintah-penting-cheat-sheet)
9. [Panduan Troubleshooting Lengkap](#-panduan-troubleshooting-lengkap)
...
- **🌐 Modul Website (`app/Modules/Website`)**:
    ...
    - **Aksesibilitas Ramah Disabilitas (WCAG)**: Font ramah disleksia (_OpenDyslexic_), mode kontras tinggi, filter buta warna (_Protanopia, Deuteranopia, Tritanopia_), pembesar teks, kursor besar.
...
- **🔗 Modul Shortlink (`app/Modules/Shortlink`)**:
    ...
    - Halaman _Lead Capture_ interaktif (judul kustom, tombol kustom, arahan kustom).
    - Pelacakan klik, pengunjung unik, perangkat, dan peramban (_analytics_).
    - Fitur Import & Export data leads (Excel/CSV).
    - **🟢 Integrasi Google Spreadsheet Real-Time & Bulk**:
        - Sinkronisasi data pengunjung/kontak masuk otomatis saat form disubmit via Webhook (Google Apps Script).
        - Sinkronisasi massal (_Bulk Sync_) dan per-baris langsung dari Filament admin panel.
        - Dukungan kustom Webhook URL per pegawai / tautan.
- **📱 Modul Tim Sosmed (`app/Modules/TimSosmed`)**:
    - Manajemen alur kerja konten media sosial (Ide, Perencanaan, Pembuatan, Review, Terbit).
    - Kalender publikasi interaktif (_Content Calendar_).
    - Manajemen beban kerja anggota tim, integrasi media platform (Instagram, TikTok, YouTube, Facebook, Twitter).
    - Obrolan tim internal (_Floating Chat Livewire_).
...
### 📊 Panduan Integrasi Google Spreadsheet (Modul Shortlink)

Fitur ini memungkinkan data pengunjung (leads) yang mengisi formulir shortlink terhubung secara **otomatis ke Google Spreadsheet** hanya dengan menyalin **1 baris rumus**. Tidak memerlukan Google Apps Script, tidak memerlukan izin akses rumit, dan tidak akan mengalami masalah error 404.

### 🚀 Cara Penggunaan (1 Menit Selesai):

1. **Buka Google Spreadsheet**:
    - Buat spreadsheet baru di [Google Sheets](https://sheets.new).
2. **Buka Admin Panel BPVP Pangkep**:
    - Masuk ke menu **Shortlink & Barcode** (`/admin/manage-shortlinks`).
    - Pada tabel **📥 Data Pengunjung / Kontak Masuk**, klik tombol hijau **`⚡ Rumus Otomatis (Tanpa Script)`**.
3. **Salin Rumus**:
    - Klik kotak **📋 Salin Rumus Google Spreadsheet Lengkap**. Rumus akan otomatis tersalin ke clipboard Anda (format: `=IMPORTDATA("https://.../shortlink/feed/leads.csv?token=...")`).
4. **Tempel di Spreadsheet**:
    - Klik sel **`A1`** (kotak paling kiri atas) di Google Spreadsheet Anda, lalu tekan `Ctrl + V` (Paste) dan tekan **Enter**.
5. **Selesai!**
    - Seluruh baris judul kolom (_Waktu Akses, Nama Pegawai, Kode Shortlink, Nama Pengunjung, WhatsApp, Email, IP, Perangkat_) beserta seluruh data pengunjung akan langsung terisi rapi secara otomatis.
    - Setiap kali spreadsheet dibuka atau diperbarui, Google Spreadsheet akan menyinkronkan data terbaru dari server BPVP Pangkep.
    - _Keamanan_: Tautan dilindungi oleh **Token Rahasia**. Jika ingin mengganti token, Anda cukup klik tombol **`🔄 Generate Ulang Token Rahasia`** di dalam modal admin.
...
- **Penyebab**: Konfigurasi `config/cache.php` memiliki `'serializable_classes' => false`, sehingga objek Eloquent Collection yang di-cache di-_unserialize_ sebagai `__PHP_Incomplete_Class`.
- **Solusi**:
